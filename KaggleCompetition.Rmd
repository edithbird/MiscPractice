...
m <- m %>% group_by(user_id) %>% mutate(timeLine = cumsum(days_since_prior_order))

```


```{r}
sampleUsers <- m %>% filter(user_id %in% sampleNumbers)
head(sampleUsers)
...
for (i in 1:length(users_split)) {
  ...
  return(ggplot(x%>% mutate(timeline = cumsum(x$days_since_prior_order)), aes(x = timeline, y = order_dow)) + geom_line() + geom_point())
